...
class TestUserCases(unittest.TestCase):
    def test_truncated_escaped_unicode_json(self):
        ...
        self.assertIn("应用经济学", result)
        self.assertEqual(result["应用经济学"]["id"], 1)
        
        self.assertIn("物理学", result)
        self.assertEqual(result["物理学"]["id"], 2)
        
        self.assertIn("天文学", result)
        self.assertEqual(result["天文学"]["id"], 23)
